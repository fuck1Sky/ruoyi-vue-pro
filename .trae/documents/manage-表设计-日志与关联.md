...
**目的**
- 记录用户与 AI 助手的每一条消息（可用 `session_id` 串起同一次对话），并记录模型、耗时与错误信息，便于审计与问题定位。

**字段设计（MySQL）**
- 业务主键
  - `id` bigint auto_increment：日志主键
- 会话/链路
  - `session_id` varchar(64)：会话编号
- 用户
  - `user_id` bigint：用户编号
...
- 图片使用 URL 地址 `image_url`（不做外键）。

**字段设计（MySQL）**
- `id` bigint auto_increment
- `activity_type` varchar(50)：活动类型（建议与字典/枚举保持一致）
- `image_url` varchar(1024)：通用图片 URL
- `sort` int：排序
- `remark` varchar(500)
...
**目的**
- 在“多活动类型”的前提下，为用户保存关联关系，并记录活动的大模型总结内容。
...
- `id` bigint auto_increment
- 用户：`user_id` bigint
- 活动：`activity_type` varchar(50)、`activity_id` bigint
- `ai_summary` longtext：活动大模型总结内容
- 通用字段（按项目规则）：`creator/create_time/updater/update_time/deleted`
...
- （可选）编写简单插入/查询用例，验证常用索引字段（`session_id/user_id/create_time`）查询命中索引。
